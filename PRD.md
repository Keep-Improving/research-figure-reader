...
- 插件自动恢复历史红框已完成第一版：解析库原文链接携带 `litfig-analysis`，插件打开页面后读取历史记录并尝试在原网页图片上恢复红框和解释。

#### M5.4 历史图像重放与离线 PDF 回溯

用户关闭原网页或 PDF 后，解析库仍应能独立回看当时的图像和解释。第一版采用“保存当时分析图像”的方式，而不是依赖原文一定可访问。

上传 PDF：

- 浏览器网页不能自动打开用户本地原文件路径，这是浏览器安全限制。
- 因此网站上传 PDF 保存解析时，必须保存当时 AI 分析用的 `figure.imageDataUrl` 和缩略图。
- 如果记录来自网站上传 PDF，还可以保存 `paper.pdfDataUrl`。这样解析库可以在网站内部打开历史 PDF，而不要求用户重新上传。
- 为避免数据膨胀，第一版保存 PDF data URL 只用于网站上传 PDF；插件网页 PDF 暂不保存整篇 PDF，只保存可见截图/图像。

历史图像重放：

- 解析库详情必须直接显示保存图像。
- 保存的 annotation bbox 应叠加回保存图像上。
- 点击或悬停红框时显示当时的 panel 解释：`what`、`howToRead`、`meaning`、`evidenceType`、`confidence`。
- 右侧仍显示完整回答、依据、不确定点和上下文。
- 如果没有保存图像，才降级为纯文本详情。

回溯按钮策略：

- 对网页记录：显示“打开原文网页”和“打开原图”。
- 对稳定 PDF URL：显示“打开 PDF 第 X 页”。
- 对网站上传 PDF 且保存了 `paper.pdfDataUrl`：显示“在网站中打开历史 PDF 第 X 页”，直接恢复到当前工作台，不要求重新上传。
